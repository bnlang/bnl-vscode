...
- Library suggestions inside `import "..."` for all 34 stdlib modules; both English (`io`) and Bangla (`ফাইল`) forms surface from any matching input.
...
- Member tables for **every** stdlib module (was 3 → now 34): `sys`, `io`, `timers`, `time`, `url`, `path`, `math`, `json`, `log`, `exec`, `web`, `request`, `mysql`, `pg`, `sqlite`, `mongo`, `random`, `uuid`, `crypto`, `csv`, `cookie`, `session`, `ws`, `test`, `net`, `http`, `tls`, `regex`, `zlib`, `dns`, `template`, `multipart`, `dotenv`, `cli`.
